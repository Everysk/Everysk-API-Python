...
class APIRequestor(object):
    ...
    def get(self, path, params):
        ...
        code, response = self.client.request('GET', url, headers=self.headers, params=params)
    ...
    def post(self, path, payload):
        url = '%s%s' % (self.base_url, path)
        code, response = self.client.request('POST', url, headers=self.headers, payload=payload)
        if code not in OK_STATUS_CODE:
            raise APIError(code, response)
        return json.loads(response)
    ...
    def delete(self, path):
        url = '%s%s' % (self.base_url, path)
        code, response = self.client.request('DELETE', url, headers=self.headers)
        if code not in OK_STATUS_CODE:
            raise APIError(code, response)
        return json.loads(response)
    ...
    def put(self, path, payload):
        url = '%s%s' % (self.base_url, path)
        code, response = self.client.request('PUT', url, headers=self.headers, payload=payload)
        if code not in OK_STATUS_CODE:
            raise APIError(code, response)
        return json.loads(response)
